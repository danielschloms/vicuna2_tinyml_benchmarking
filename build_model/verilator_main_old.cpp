...
int main(int argc, char **argv) {
  ...
  while (getline(&line, &line_sz, fprogs) > 0) {
    ...
    // simulate program execution
    {
      ...
      bool main_reached = false; // detect if main has been reached to begin
      ...
      while (end_cnt < extra_cycles) {
        ...
        main_reached = (current_IF_PC == 0x00002000u) |
        ...
        if (cycles_stalled >= 10000) { // TODO: Expose this to the command line
          fprintf(
              stderr,
              "ERROR: SIMULATION STALLED FOR 10000 CYCLES AT IF_PC = 0x%x\n",
              current_IF_PC);
          exit_code = 1;
          break;
        }
